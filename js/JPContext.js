...
JPMethodContext.prototype.parse = function(){
	var ctx = this;
	// var funcName = this.ignore ? '' : this.names.join('_');
	// if (this.names.length == this.params.length) funcName+='_';

	// var script =  funcName + ": function(" + this.params.join(',') + ") {"
	if (!this.ignore) {
		...
		if (this.params.join(',') != '') this.names.push('');
